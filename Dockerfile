...
RUN sed -i 's|VITE_API_BASE_URL=.*|VITE_API_BASE_URL=https://build-a-responsive-web-app-called.launchpulse.ai|g' .env 2>/dev/null || true
RUN sed -i 's|VITE_BACKEND_URL=.*|VITE_BACKEND_URL=https://build-a-responsive-web-app-called.launchpulse.ai|g' .env 2>/dev/null || true
RUN echo 'VITE_API_BASE_URL=https://build-a-responsive-web-app-called.launchpulse.ai' > .env.production.local && echo 'VITE_BACKEND_URL=https://build-a-responsive-web-app-called.launchpulse.ai' >> .env.production.local
RUN npm run build
